...
type service struct {
	...
	log *logger.Logger // uber's structured and leveled logger
	{{ if .DB }}db  *pg.DB             // postgresql database connection{{ end }}
	{{ if .Jobs }}wp  *work.Enqueuer     // redis worker pool{{ end }}
}

// New service factory
func New(log *logger.Logger{{ if .DB }}, db *pg.DB{{ end }}{{ if .Jobs }}, wp *work.Enqueuer{{ end }}) {{ package .ServiceName }}.ServiceServer {
